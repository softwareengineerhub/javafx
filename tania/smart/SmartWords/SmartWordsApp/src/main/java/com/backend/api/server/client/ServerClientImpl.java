...
public class ServerClientImpl implements ServerClient {
    ...
    public Object readData(MessageData requestData) {
        if (requestData.isCategoryRequest()) {
            try (ObjectInputStream in = new ObjectInputStream(getClass().getResourceAsStream("/categories/cat" + requestData.getCategoryPaginationIndex() + ".txt"))) {
